...
namespace AWSFileUploaderWithImageCompression.Classes
{
    public class ImageService : IImageService
    {
        ...
        public async Task<PutObjectResponse> CompressAndUploadImageAsync(Stream sourceImageStream, string fileKeyName = "")
        {
            var tmpFile = Path.GetTempFileName();
            var comp = await imageComp.CompressImage(sourceImageStream, tmpFile);
            if (comp.ImageCompressionSucccess)
                return await s3Uploader.UploadAsync(File.OpenRead(tmpFile), key: fileKeyName);
            else
                throw new Exception("Image compression failed.");
        }

        public async Task<PutObjectResponse> CompressAndUploadImageAsync(string sourceFilePath, string fileKeyName = "")
        {
            return await CompressAndUploadImageAsync(File.OpenRead(sourceFilePath), fileKeyName);
        }

        public async Task<PutObjectResponse> CompressWaterMarkAndUploadAsync(Stream sourceImageStream, Stream watermarkImage, string fileKeyName = "")
        {
            var tmpFile = Path.GetTempFileName();
        ...
        public async Task<PutObjectResponse> CompressWaterMarkAndUploadAsync(string sourceFilePath, Stream watermarkImage, string fileKeyName = "")
        {
